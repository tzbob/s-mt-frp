...
import mtfrp.html.lang.HtmlNodeBuilderLib
import scala.js.exp.JSMapsExp
import scala.js.exp.dom.{ElementOpsExp, EventOpsExp}
...
trait HtmlNodeBuilderLibExp extends HtmlNodeBuilderLib with ClientFRPLibExp with EventOpsExp with JSMapsExp with ElementOpsExp {
  ...
  case class MkNode(
    ...
    children: Exp[List[HtmlNode]]
  ) extends Def[HtmlNode]
  def mkNode(
    tagName: Exp[String],
    handlers: List[Handler],
    attributes: Attributes,
    children: Rep[List[HtmlNode]]
  ): Exp[HtmlNode] = {
